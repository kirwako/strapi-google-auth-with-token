...
export default ({ strapi }: { strapi: Strapi }) => ({
  ...
  async createGoogleCredentials(body: IAddGoogleCredentialBody) {
    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      try {
        ...
        if (!credentials) {
          await strapi.entityService.create(googleCredentialEntityService, {
            data: {
  ...
  async auth(body: { token: string }) {
    ...
    if (!authUser || !authUser.length) {
      ...
      const sanitizedUser = await sanitizeUser(createdUser);
      ...
      const sanitizedUser = await sanitizeUser(authUser);
